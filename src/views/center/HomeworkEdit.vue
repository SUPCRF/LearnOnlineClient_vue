<template>
  <div>
    ...
    <div>
      ...
      <el-dialog title="编辑修改作业" :visible.sync="modifFormVisible">
        ...
        <div slot="footer" class="dialog-footer">
          ...
          <el-button type="primary" @click="modifHomework">确 定</el-button>
        </div>
      </el-dialog>
    </div>
  </div>
...
@Component()
export default class HomeworkEdit extends Vue {
  data() {
    return {
      tableData: [],
  ...
  delClick(index) {
    this.$confirm("此操作将永久删除该作业, 是否继续?", "提示", {
      ...
      .then(() => {
        ...
        axios({
          url: "http://localhost:8000/api/homework-content/delHomework",
          method: "post",
          data: formdata,
          headers: { Authorization: token },
        })
  ...
  addHomework() {
    ...
    axios({
      url: "http://localhost:8000/api/homework-content/saveById",
      method: "post",
      data: formdata,
      headers: { Authorization: token },
    })
  ...
  modifHomework() {
    ...
    axios({
      url: "http://localhost:8000/api/homework-content/modifHomework",
      method: "post",
      data: formdata,
      headers: { Authorization: token },
    })
